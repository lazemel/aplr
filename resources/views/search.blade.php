...
<body class="flex items-center justify-center min-h-screen bg-gray-100">
    <form action="{{ route('documents.search', [], true) }}" method="GET" class="max-w-lg w-full">
        ...
        @if(isset($documents))
            ...
            @else
                <ul class="space-y-4 bg-white p-4 shadow-lg rounded-lg">
                    @foreach($documents as $document)
                        <li class="p-2 border-b border-gray-200 hover:bg-gray-100 transition duration-150">
                            <span class="font-medium text-blue-600">{{ $document->title }}</span>
                            <span class="text-sm text-gray-500">by {{ $document->author ?? 'Unknown Author' }}</span>
                        </li>
                    @endforeach
                </ul>
            @endif
